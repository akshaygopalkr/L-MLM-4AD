...
import torch
import argparse
# from modules.multi_frame_dataset_rag import MultiFrameDataset
from modules.dataset.multi_frame_dataset_rag import MultiFrameDataset
from modules.models.multi_frame_rag import DriveVLMT5
...
def val_model(dloader, val_model):
    ...
    for step, (inputs, imgs, labels, q_texts, img_paths) in tqdm(enumerate(dloader),
                                                                 total=len(dloader)):
        outputs = val_model(inputs, imgs, q_texts, img_paths, labels)
...
def custom_train(train_loss, val_loss, best_model, epochs, learning_rate):
    ...
    for epoch in range(epochs, config.epochs):
        ...
        for step, (inputs, imgs, labels, q_texts, img_paths) in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):
        # for step, (inputs, imgs, labels) in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):

            # Forward pass through model
            outputs = model(inputs, imgs, q_texts, img_paths, labels)
            # outputs = model(inputs, imgs, None, None, labels)
        ...
        save_model(model, 'latest_model_2')
        epochs += 1
...
def save_experiment(statistics):
    ...
    trial_dict = {
        ...
        'RAG-Enabled': [config.rag],
        'RAG-Contrast': [config.rag_contrast],
...
def params():
    ...
    parser.add_argument('--rag-contrast', action='store_true', help='Use contrastive examples for RAG')
    parser.add_argument('--img-encoder', type=str, choices=['CLIP', 'Patch'], help='Image encoder '
                                                                                            'to be used for the network')
    parser.add_argument('--num-context', type=int, default=2, help='# of context examples for RAG')
    parser.add_argument('--rag', action='store_true', help='Using RAG for answer generations')
...
if __name__ == '__main__':
    ...
    model = DriveVLMT5(config, processor)
    model.to(device)
    print('Trainable Parameters for full model')

    train_dset = MultiFrameDataset(
        input_file=os.path.join('data', 'multi_frame_rag',
    ...
    if not config.hf_train:
        ...
        if config.load_checkpoint:
            ...
            # model = torch.load(os.path.join('multi_frame_results', config.checkpoint_file, 'latest_model.pth'))
            # model.rag = True
            # model.rag_model = MFRAG(config, processor)
            # best_model = torch.load(os.path.join('multi_frame_results', config.checkpoint_file,
            #                                                    'latest_model.pth'))
            # best_model.rag = True
            # best_model.rag_model = MFRAG(config, processor)
            # model.to(device), best_model.to(device)
            ...
            model.load_state_dict(torch.load(os.path.join('multi_frame_results', config.checkpoint_file,
                                                          'latest_model.pth')))
            best_model = DriveVLMT5(config, processor)
        ...
        best_model = DriveVLMT5(config, processor)
        best_model.load_state_dict(torch.load(os.path.join('multi_frame_results', timestr, 'latest_model_2.pth')))
